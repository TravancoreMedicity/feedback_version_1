...
import DischargePatientList from './InpatientDischargeDetails/DischargePatientList';
...
const PateintRoomDetail = ({ beddetail, nsname, view, setView, nscode }) => {
    ...
    const [loadingBed, setLoadingBed] = useState(null); // bdcode | null
    const [activeTab, setActiveTab] = useState('BED');
    const [nsdischargePatients, setNsDichargePatients] = useState([]);
    const [loadingdispat, setLoadingDisPat] = useState(false)
    // const [loadingrefresh, setLoadingRefresh] = useState(false)
    // const [isCoolingDown, setIsCoolingDown] = useState(false);

    //Prevent duplicate calls for same tab
    ...
    const GetPatientDetailFromMeliora = useCallback(async (bdcode) => {
        ...
        try {
            ...
            if (success === 1) {
                ...
                setInpatientDetailFromMeliora({})
                setLoading(false)
                infoNofity("Patient Detail Missing...")
                return;
            }
            if (success !== 2) {
    ...
    // Fetch Discharge Patients of the Particular Nursing Stations
    const FetchDischargePatients = async () => {

        setLoadingDisPat(true)
        try {
            const Response = await axiosApi.post('/feedback/dischargedpatient', {
                fb_ns_code: nscode
            });
            ...
            if (success !== 2) {
                warningNofity("Error in Fetching Discharge PatientDetails")
                return
            }
            setNsDichargePatients(data ?? data)
            succesNofity(message)
        } catch (error) {
            errorNofity("Error in Fetching Dicharge Patients", error)
        } finally {
            setLoadingDisPat(false)
        }
    }


    useEffect(() => {
        if (activeTab === 'BED' || fetchedRef.current) return;
        fetchedRef.current = true;
        FetchDischargePatients();
    }, [activeTab, FetchDischargePatients]);
                    ...
                    <PatientTabContent
                                                                            ...
                                                                            <Grid xs={6} sm={3} lg={2} xl={1.5} key={index} >
                                                                                <Suspense fallback={<CustomBackDropWithOutState message={"Loading..."} />} >
                                                                                    <RoomComponent
                                                                                        ...
                                                                                        open={open}
                                                                                        setOpen={setOpen}
                                                                                        BedLoading={loadingBed === item?.fb_bd_code}
                                                                                    />
                                                                                </Suspense>
                                                                            </Grid>
                                                                        )
                        ...
                        dischargeView={
                            <Box sx={{ p: 2, minHeight: '80vh' }}>
                                {/* FUTURE DISCHARGED PATIENT UI */}
                                <DischargePatientList
                                    dischargePatient={nsdischargePatients}
                                    loadingDischargePatient={loadingdispat}
                                // feedbackedexit={patientfeedbackdata}
                                />
                            </Box>
                        }
