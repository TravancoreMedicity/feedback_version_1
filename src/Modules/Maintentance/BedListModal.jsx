...
const BedListModal = ({ open,
    ...
    const HandleBedDetailRequest = useCallback(async () => {
        ...
        if (empid.length === 0) return warningNofity("Please select the Employee");
        if (checkcomplaint?.length > 0 && !isinitalchecked) return warningNofity("Please Complete the Intial CheckList");
        if (checkcomplaint?.length !== filteredAssets?.length) return warningNofity("Please Register Complaint");
        if (filteredAssets?.length !== RectifiedComplaint?.length) return warningNofity("Please Rectify Complaint");
        ...
        <Box>
            <Modal
                ...
                <ModalDialog sx={{
                    ...
                    <ModalHeader name={'CHECK LIST'} data={data} HanldeModalClose={HanldeModalClose} />
                    {
